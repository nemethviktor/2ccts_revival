...
def generate_vehiclesort_pnml():
    ...
    output_path = os.path.normpath(os.path.join(
        project_root, 'src/vehiclesort.pnml'))
    ...
    for _, r_row in df_ranges.iterrows():
        ...
        cat_items = df_master[df_master['VEHID_ID_CATEGORY'].astype(
            str).str.strip() == cat_id].copy()
        ...
        for i, (_, p_row) in enumerate(cat_items.iterrows()):
            ...
            year = int(p_row['INTRODUCTION_YEAR']) if pd.notna(
                p_row['INTRODUCTION_YEAR']) else 0
