...
def get_git_info():
    try:
        ...
        # Calculate days since 2000-01-01 (Matches findversion.sh VERSION logic)
        commit_date = date.fromtimestamp(time.time())
    ...
    try:
        ...
        for line in lines:
            ...
            if re.search(r'^\s*version:', line):
                ...
                line = re.sub(
                    r'(^\s*version:\s+)([^\s;]+)', r'\g<1>' + new_version, line)
